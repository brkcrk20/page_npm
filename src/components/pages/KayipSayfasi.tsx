/**
 * Kayıp & bulundu listesi.
 *
 * Gövde rota dosyasından ayrıldı: /kayip süzgeç için searchParams okuyordu
 * ve searchParams okuyan rota Next'te tamamen dinamik sayılıyor — yanıt
 * "no-store" ile çıkıyor, CDN'e hiç uğramıyor. Ölçüm: TTFB 503 ms, oysa
 * önbelleğe giren diğer sayfalar 70-100 ms.
 *
 * Temiz adres (/kayip) artık süzgeç okumuyor ve önbelleğe giriyor; sekme
 * ve şehir seçilince istek /kayip-filtre altına yeniden yazılıyor. Aynı
 * çözümün ilan ve hizmet listelerindeki hâli için bkz. ListeSayfalari.tsx.
 */
...
export type KayipSuzgeci = { tip?: string; sehir?: string };

export async function KayipListesi({ sp }: { sp: KayipSuzgeci }) {
